...
- **TIMESTAMP time zone** -- `TIMESTAMP` columns are stored as UTC internally but returned through the session `time_zone`. Retrieved instants are only guaranteed correct when the session time zone is UTC. The connector pre-declares `SET time_zone = '+00:00'` on every new connection via the `session_init_sql` dialect hook (pending CDK engine integration); ensure the MySQL server's global `time_zone` is set to `'+00:00'` until that hook is active.
